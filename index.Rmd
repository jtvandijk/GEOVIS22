...
## Tasks {#task-s3}
We now have created a Kernel Density Estimation for the bearers of *Van Dijk* using two different bandwidths.  Feel free to play around with different colours, settings and visualisation options (e.g. when visualising the output of the **Kernel Density Estimation** geoprocessing tool). In addition, using either the *Van Dijk* dataset, or one of the other dataset, try to see how the results are affected:
